...
SOURCE = ROOT / "research/diagnostics/manuscript_v3_f120_initializer_error_direction_freeze_v1.py"
SPEC = importlib.util.spec_from_file_location("f120_freeze", SOURCE)
...
def payload(upper=(1, 2), threshold=(1, 2)):
    return {
        ...
        "f121_threshold": {"denominator": threshold[1], "numerator": threshold[0]},
...
def refusal(value):
    with pytest.raises(M.F120Refusal) as exc:
        M.evaluate_initializer_error_gate(value)
...
def test_exact_comparator(upper, threshold, decision):
    got = M.evaluate_initializer_error_gate(payload(upper, threshold))
...
@pytest.mark.parametrize("key,value", [
    ("metric_index", 2), ("metric_index", 4), ("metric_index", True),
    ("metric_index", 3.0), ("metric_id", "Initializer-error"),
    ("metric_id", "initializer-error "), ("direction", "LOWER_BOUND"),
    ("direction", "upper_bound"), ("scalar_definition_sha256", "A" * 64),
    ("f121_threshold_record_sha256", "2" * 63),
])
...
@pytest.mark.parametrize("slot,bad", [
    ...
    ("f121_threshold", {"denominator": 4, "numerator": 2}),
    ("f121_threshold", {"numerator": 1, "denominator": 2}),
])
...
def test_missing_extra_and_reordered_inputs_refuse():
    x = payload(); x.pop("f121_threshold"); refusal(x)
...
@pytest.mark.parametrize("mutate", [
    ...
    lambda d: d.update(state="F120_FAKE"),
    ...
    lambda d: d["project_effects_and_nonclaims"].update(F121_F127_remain_open=False),
...
@pytest.mark.parametrize("mutate", [
    lambda d: d["field_closures"].append({"field_id":"F121","json_pointer":"/metric_and_estimand_plan/constraint_metrics/3/threshold_or_margin","owner_role":"OWNER_A_THEORY_AND_STATISTICS","value":"1e-6"}),
    lambda d: d["field_closures"][0].update(field_id="F119"),
    lambda d: d["field_closures"][0].update(json_pointer="/metric_and_estimand_plan/constraint_metrics/4/direction"),
    lambda d: d["field_closures"][0].update(value="MAX_KL_TV"),
    lambda d: d["count_transition"]["after"].update(pre_execution_open=140),
    lambda d: d["workstream_transition"]["after"]["theory_statistics"].update(closed=22),
    ...
    lambda d: d["direction_contract"].update(f121_threshold_selected_here=True),
...
def test_predecessor_semantic_resigning_fails(tmp_path):
    root = clone_package(tmp_path)
    p = root / "research/fixtures/manuscript_v3_f145_validation_early_stopping_freeze_v1.json"
...
def test_package_roster_and_bindings_are_receipt_lifecycle_stable():
    assert M.PACKAGE_ROSTER == [
        "PROJECT_F120_INITIALIZER_ERROR_DIRECTION_FREEZE.md",
        "research/fixtures/manuscript_v3_f120_initializer_error_direction_freeze_v1.json",
        "research/diagnostics/manuscript_v3_f120_initializer_error_direction_freeze_v1.py",
        "tests/unit/test_manuscript_v3_f120_initializer_error_direction_freeze_v1.py",
    ]
    record = json.loads((ROOT / M.MACHINE_PATH).read_text())
    current_paths = [row["path"] for row in record["package_bindings_excluding_machine_self"]]
    receipt_names = {
        "PROJECT_F120_INITIALIZER_ERROR_DIRECTION_FREEZE_INDEPENDENT_REVIEW.md",
