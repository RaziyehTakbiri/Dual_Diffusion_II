# CP50 Test-28 mixed-support initializer protocol v1 — DRAFT sidecar v22
...
This file is the additive v22 DRAFT sidecar.  Its scientific protocol
identifier remains `cp50-test28-mixed-initializer-v1`; the sidecar generation
does not create a new scientific protocol, rewrite the immutable v21 protocol
or manifest, freeze either v22 path, or authorize execution.  CP71 binds the
exact v21 bytes as predecessor-only inputs and requires its own source hashes
to be supplied by this later manifest.  That hash pin does not change v21's
`DRAFT` state, constitute a freeze receipt, make either sidecar authoritative
for production, or make v22 an input consumed by CP71.  The proposed v15
lifecycle graph remains nonauthoritative for production.
...
### 10.1 Completed CP52--CP71 nonconfirmatory prerequisites
...
estimate/interval recomputation qualification source/test evidence.  None
of these sidecars is a freeze receipt or production authorization.  CP65
consumes v15, CP66 consumes v16, CP67 consumes v17, CP68 consumes v18, CP69
consumes v19, CP70 consumes v20, and CP71 consumes v21 only as predecessor
custody; none consumes v22 or makes the proposed lifecycle authoritative.  Any byte change requires
...
- The completed twenty-seven-path CP50--CP71 aggregate passes 1,890/1,890 tests
  with warnings treated as errors and no warning, error, or failure output.
...
The DRAFT blocker ledger now contains twenty-six entries: twenty-two satisfied by
...
CP71 are completed development precursors to that blocker; the missing set is
unchanged.
...
- unconditional operational predictions under the selected correlated model:
  ...
  production estimate, interval, or decision;
- the execution runner, source/fixture/seed manifests, exact dependency lock,
  ...
  receipts, gates 13 and 14, and the production runner remain absent;
