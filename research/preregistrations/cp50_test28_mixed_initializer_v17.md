# CP50 Test-28 mixed-support initializer protocol v1 — DRAFT sidecar v17
...
This file is the additive v17 DRAFT sidecar.  Its scientific protocol
identifier remains `cp50-test28-mixed-initializer-v1`; the sidecar generation
does not create a new scientific protocol, rewrite the immutable v16 protocol
or manifest, freeze either v17 path, or authorize execution.  CP66 binds the
exact v16 bytes as predecessor-only inputs and requires its own source hashes
to be supplied by this later manifest.  That hash pin does not change v16's
`DRAFT` state, constitute a freeze receipt, make either sidecar authoritative
for production, or make v17 an input consumed by CP66.  The proposed v15
...
### 10.1 Completed CP52--CP66 nonconfirmatory prerequisites
...
evidence.  None of these sidecars is a freeze receipt or production
authorization.  CP65 consumes v15 and CP66 consumes v16 only as predecessor
custody; neither consumes v17 or makes the proposed lifecycle authoritative.
Any byte change requires renewed testing and a new manifest binding before
freeze review.
...
- The current twenty-two-suite CP50--CP66 aggregate passes 1,558/1,558 tests
  with warnings treated as errors and no warning, error, or failure output.
...
The DRAFT blocker ledger now contains twenty-one entries: seventeen satisfied by
...
- unconditional operational predictions under the selected correlated model:
  ...
  performs no production execution;
- the execution runner, source/fixture/seed manifests, exact dependency lock,
  ...
  independent production recomputation remain absent;
