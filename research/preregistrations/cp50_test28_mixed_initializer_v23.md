# CP50 Test-28 mixed-support initializer protocol v1 — DRAFT sidecar v23
...
This file is the additive v23 DRAFT sidecar.  Its scientific protocol
identifier remains `cp50-test28-mixed-initializer-v1`; the sidecar generation
does not create a new scientific protocol, rewrite the immutable v22 protocol
or manifest, freeze either v23 path, or authorize execution.  CP72 binds the
exact v22 bytes as predecessor-only inputs and requires its own source hashes
to be supplied by this later manifest.  That hash pin does not change v22's
`DRAFT` state, constitute a freeze receipt, make either sidecar authoritative
for production, or make v23 an input consumed by CP72.  The proposed v15
...
### 10.1 Completed CP52--CP72 nonconfirmatory prerequisites
...
CP71 consumes v21, and CP72 consumes v22 only as predecessor custody; none
consumes v23 or makes the proposed lifecycle authoritative.  Any byte change
...
- The exact focused CP72 suite passes 131/131 tests with warnings treated as
  ...
  This focused receipt is nonconfirmatory development verification only.  It
  supplies no production or confirmatory evidence and authorizes no execution.
- The completed twenty-eight-path CP50--CP72 aggregate passes 2,021/2,021
...
The DRAFT blocker ledger now contains twenty-seven entries: twenty-three satisfied by
...
to that blocker; the missing set is unchanged.
...
- unconditional operational predictions under the selected correlated model:
  ...
  prediction, estimate, interval, or decision;
