# CP50 Test-28 mixed-support initializer protocol v1 — DRAFT sidecar v16
...
This file is the additive v16 DRAFT sidecar.  Its scientific protocol
identifier remains `cp50-test28-mixed-initializer-v1`; the sidecar generation
does not create a new scientific protocol, rewrite the immutable v15 protocol
or manifest, freeze either v16 path, or authorize execution.  CP65 binds the
exact v15 bytes as predecessor-only inputs and requires its own source hashes
to be supplied by this later manifest.  That hash pin does not change v15's
`DRAFT`/`DRAFT_PRE_FREEZE` state, constitute a freeze receipt, make either
sidecar authoritative for production, or make v16 an input consumed by CP65.
The proposed v15 lifecycle graph remains nonauthoritative for production.
...
### 10.1 Completed CP52--CP65 nonconfirmatory prerequisites
...
v16 DRAFT pair binds the additive CP65 authoritative and source-independent
source/test evidence.  None of these sidecars is a freeze receipt or
production authorization.  CP65 consumes v15 only as predecessor custody; it
does not consume v16 or make the proposed lifecycle authoritative.  Any byte
change requires renewed testing and a new manifest binding before freeze
review.
...
- The current twenty-one-suite CP50--CP65 aggregate passes 1,449/1,449 tests
  with warnings treated as errors and no warning, error, or failure output.
...
The DRAFT blocker ledger now contains twenty entries: sixteen satisfied by
...
production blocker.  The missing set is unchanged.
...
- unconditional operational predictions under the selected correlated model:
  ...
  receipt, request, campaign, estimate, interval, or decision;
- the execution runner, source/fixture/seed manifests, exact dependency lock,
  ...
  retained records, and the overall production schema remain absent;
