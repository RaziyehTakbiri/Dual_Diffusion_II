# CP50 Test-28 mixed-support initializer protocol v1
...
or environment may change after `STARTED.json`. An infrastructure abort before
any stochastic output becomes durable may receive a new attempt number only
after written independent adjudication and with identical frozen inputs. Once
stochastic output is durable, the confirmatory attempt is spent. A failed v1
may motivate a separately named v2 but cannot be rewritten or replaced.
...
### 10.1 Completed CP52--CP63 nonconfirmatory prerequisites

The accompanying machine-readable DRAFT manifest is the sole digest authority
for the following completed source-and-test surfaces; their hashes and byte
counts are not duplicated in this prose. Any byte change requires renewed
...
with
`tests/unit/test_mixed_initializer_test28_independent_recomputation.py`.
The
manifest also
...
- The CP62 calibration-only execution-capsule pair passes 148/148 focused
  ...
  counts, and the exact focused command and timing.  The current sixteen-suite
...
- The current eighteen-suite CP50--CP63 aggregate passes 1,066/1,066 tests
...
The DRAFT blocker ledger now contains eighteen entries: fourteen satisfied by
...
blocker or `unconditional_operational_predictions`.
...
- unconditional operational predictions under the selected correlated model:
  ...
  sample, estimate, interval, or decision;
- the execution runner, source/fixture/seed manifests, exact dependency lock,
  ...
  estimate, interval, or decision is bound;
- a completed power review, exact frozen primary thresholds, and adequate
  selected-count justification for every one of the 32 gate slots; and
- complete confirmatory custody: sidecar protocol digest, freeze receipt,
  ...
  machinery, and explicit launch authorization.
