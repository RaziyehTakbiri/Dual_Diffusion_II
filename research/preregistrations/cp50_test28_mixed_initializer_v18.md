# CP50 Test-28 mixed-support initializer protocol v1 — DRAFT sidecar v18
...
This file is the additive v18 DRAFT sidecar.  Its scientific protocol
identifier remains `cp50-test28-mixed-initializer-v1`; the sidecar generation
does not create a new scientific protocol, rewrite the immutable v17 protocol
or manifest, freeze either v18 path, or authorize execution.  CP67 binds the
exact v17 bytes as predecessor-only inputs and requires its own source hashes
to be supplied by this later manifest.  That hash pin does not change v17's
`DRAFT` state, constitute a freeze receipt, make either sidecar authoritative
for production, or make v18 an input consumed by CP67.  The proposed v15
...
### 10.1 Completed CP52--CP67 nonconfirmatory prerequisites
...
and CP67 consumes v17 only as predecessor custody; none consumes v18 or makes
...
- The current twenty-three-suite CP50--CP67 aggregate passes 1,589/1,589 tests
  with warnings treated as errors and no warning, error, or failure output.
...
The DRAFT blocker ledger now contains twenty-two entries: eighteen satisfied by
...
CP63, CP66, and CP67 are completed development precursors to that blocker; the
