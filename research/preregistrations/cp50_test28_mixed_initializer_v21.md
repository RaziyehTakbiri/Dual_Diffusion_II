# CP50 Test-28 mixed-support initializer protocol v1 — DRAFT sidecar v21
...
This file is the additive v21 DRAFT sidecar.  Its scientific protocol
identifier remains `cp50-test28-mixed-initializer-v1`; the sidecar generation
does not create a new scientific protocol, rewrite the immutable v20 protocol
or manifest, freeze either v21 path, or authorize execution.  CP70 binds the
exact v20 bytes as predecessor-only inputs and requires its own source hashes
to be supplied by this later manifest.  That hash pin does not change v20's
`DRAFT` state, constitute a freeze receipt, make either sidecar authoritative
for production, or make v21 an input consumed by CP70.  The proposed v15
...
### 10.1 Completed CP52--CP70 nonconfirmatory prerequisites
...
reducer and exact-output validator qualification source/test evidence.  None
...
consumes v19, and CP70 consumes v20 only as predecessor custody; none consumes
v21 or makes the proposed lifecycle authoritative.  Any byte change requires
...
- The historical twenty-five-suite CP50--CP69 aggregate passes 1,723/1,723 tests
  ...
  it supplies no production or confirmatory evidence and authorizes no
  execution.
- The current twenty-six-suite CP50--CP70 aggregate passes 1,808/1,808 tests
  with warnings treated as errors and no warning, error, or failure output.
...
The DRAFT blocker ledger now contains twenty-five entries: twenty-one satisfied by
...
and CP70 are completed development precursors to that blocker; the missing set
is unchanged.
...
- unconditional operational predictions under the selected correlated model:
  ...
  sample, and supplies no production estimate, interval, or decision;
