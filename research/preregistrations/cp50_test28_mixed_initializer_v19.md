# CP50 Test-28 mixed-support initializer protocol v1 — DRAFT sidecar v19
...
This file is the additive v19 DRAFT sidecar.  Its scientific protocol
identifier remains `cp50-test28-mixed-initializer-v1`; the sidecar generation
does not create a new scientific protocol, rewrite the immutable v18 protocol
or manifest, freeze either v19 path, or authorize execution.  CP68 binds the
exact v18 bytes as predecessor-only inputs and requires its own source hashes
to be supplied by this later manifest.  That hash pin does not change v18's
`DRAFT` state, constitute a freeze receipt, make either sidecar authoritative
for production, or make v19 an input consumed by CP68.  The proposed v15
...
### 10.1 Completed CP52--CP68 nonconfirmatory prerequisites
...
CP67 consumes v17, and CP68 consumes v18 only as predecessor custody; none
consumes v19 or makes the proposed lifecycle authoritative.  Any byte change
requires renewed testing and a new manifest binding before freeze review.
...
- The historical twenty-three-suite CP50--CP67 aggregate passes 1,589/1,589 tests
  ...
  final, and no historical verification record is rewritten.
- The current twenty-four-suite CP50--CP68 aggregate passes 1,646/1,646 tests
  with warnings treated as errors and no warning, error, or failure output.
...
The DRAFT blocker ledger now contains twenty-three entries: nineteen satisfied by
...
- the execution runner, source/fixture/seed manifests, exact dependency lock,
  ...
  gates 13 and 14, and the production runner remain absent;
