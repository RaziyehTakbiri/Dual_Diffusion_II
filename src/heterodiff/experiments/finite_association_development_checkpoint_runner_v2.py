"""One-attempt V2 recovery execution for one frozen finite-A1 checkpoint.

V2 is a transparent same-coordinate recovery after V1 failed before any
optimizer update because of a module-identity defect.  It does not extend or
impersonate the A1 production order.  It copies the complete decision-bearing
source closure into a new durable development capsule and invokes the existing
sampled runner inside it.  ``artifacts/a1_campaign_v4`` is created only below the V2 capsule, never at the repository production path.  The immutable V1
failure custody is revalidated throughout the attempt.
...
SCHEMA_VERSION = "heterodiff-a1-development-checkpoint-runner-v2"
RECEIPT_SCHEMA_VERSION = "heterodiff-a1-development-checkpoint-receipt-v2"
...
ARTIFACT_RELATIVE_PATH = "artifacts/manuscript_v3_a1_development_checkpoint_v2"
...
V1_ARTIFACT_RELATIVE_PATH = "artifacts/manuscript_v3_a1_development_checkpoint_v1"
V1_MACHINE_FREEZE_RELATIVE_PATH = (
    "research/fixtures/manuscript_v3_a1_development_checkpoint_freeze_v1.json"
)
V1_FAILURE_RECEIPT_RELATIVE_PATH = "failure-receipt.json"
V1_LEDGER_RELATIVE_PATH = "capsule/artifacts/a1_campaign_v4/ledger.json"
V1_INNER_STDERR_RELATIVE_PATH = "inner-run.stderr"
V1_RETAINED_FREEZE_RELATIVE_PATH = "authorized-freeze.json"
V1_ARTIFACT_INVENTORY_DOMAIN = b"heterodiff-a1-development-artifact-inventory-v1\0"
V1_EXPECTED_REGULAR_FILE_COUNT = 269
V1_EXPECTED_TOTAL_REGULAR_FILE_BYTES = 16_474_540
V1_EXPECTED_ARTIFACT_INVENTORY_SHA256 = (
    "c371a749a025527d8f34a305cca29da57f1ecfaf60ffc8d770d8e0c6866adbd9"
)
V1_EXPECTED_FAILURE_RECEIPT_RAW_SHA256 = (
    "005772f7dbbe5fc43696bd1f56dca4ec33f15bcc8bc4683558aa597260e4b721"
)
V1_EXPECTED_FAILURE_RECEIPT_RECORD_SHA256 = (
    "fb8019f05982a128fe8a5c5d6bc3d60b6e009eece936afd053b7bbe8025f6e2b"
)
V1_EXPECTED_LEDGER_RAW_SHA256 = (
    "e9df396654a7763157259e4ace1c77eee6e0c1555881cdb1be5e1cab8e6f5016"
)
V1_EXPECTED_LEDGER_RECORD_SHA256 = (
    "f63dfb6a7d8246b099ee059b6db3bb784cb269c647a70b4d6c3f26eaa32495ed"
)
V1_EXPECTED_INNER_STDERR_RAW_SHA256 = (
    "b1fdeaeec243e4f518930f7f4f2391b491b2c71bd14d3040085fcb459a3d0221"
)
V1_EXPECTED_MACHINE_FREEZE_RAW_SHA256 = (
...
RETAINED_MODULE_IDENTITY_TEST_FILE_NAME = "authorized-module-identity-test.py"
...
FREEZE_DOCUMENT_RELATIVE_PATH = "manuscript_v3/a1_development_checkpoint_freeze_v2.md"
...
STATIC_FREEZE_SEMANTIC_SHA256 = (
    "281420b0a97a9266ebcd2b057b8a7918c72fc144a31b49ac47c60d5fb1defa20"
)
...
_LAUNCH_GATE_ENVIRONMENT_NAME = "HETERODIFF_A1_DEVELOPMENT_V2_LAUNCH_GATE_FD"
_LAUNCH_GATE_TOKEN = b"HETERODIFF_A1_DEVELOPMENT_LAUNCH_GO_V2\n"
_LAUNCH_GATE_TOKEN_SHA256 = hashlib.sha256(_LAUNCH_GATE_TOKEN).hexdigest()
_ISOLATED_RUNNER_GATE_BOOTSTRAP = (
    "import os\n"
    "import runpy\n"
    "fd=int(os.environ.pop('HETERODIFF_A1_DEVELOPMENT_V2_LAUNCH_GATE_FD'))\n"
    "payload=bytearray()\n"
    "while True:\n"
    ...
    "os.close(fd)\n"
    "if bytes(payload)!=b'HETERODIFF_A1_DEVELOPMENT_LAUNCH_GO_V2\\n':\n"
    "    raise SystemExit(73)\n"
    "runpy.run_module("
...
_SUCCESS_RECEIPT_FIELDS = frozenset(
    {
        ...
        "runner_test_sha256",
        "isolated_runner_source_sha256",
        "module_identity_test_sha256",
...
def _require_authorized_freeze(root: Path) -> Dict[str, Any]:
    ...
    if set(binding) != {
        ...
        "runner_test_path",
        "runner_test_sha256",
        "isolated_runner_source_path",
        "isolated_runner_source_sha256",
        "module_identity_test_path",
        "module_identity_test_sha256",
        "source_manifest_sha256",
        "training_configuration_sha256",
    }:
    ...
    expected_paths = {
        ...
        "isolated_runner_source_path": ISOLATED_RUNNER_RELATIVE_PATH,
        "module_identity_test_path": MODULE_IDENTITY_TEST_RELATIVE_PATH,
    }
    ...
    if (
        ...
        or binding.get("isolated_runner_source_sha256")
        != _sha256_file(root / ISOLATED_RUNNER_RELATIVE_PATH, maximum=64 * 1024 * 1024)
        or binding.get("module_identity_test_sha256")
        != _sha256_file(
            root / MODULE_IDENTITY_TEST_RELATIVE_PATH, maximum=64 * 1024 * 1024
        )
    ):
    ...
    observed_v1_custody = _require_v1_failure_custody(root)
    if freeze.get("v1_failure_custody") != observed_v1_custody:
        raise DevelopmentCheckpointRefusal("V2 freeze V1 failure custody changed")
    recovery = freeze.get("recovery_amendment")
    if (
        type(recovery) is not dict
        or recovery.get("amendment_class")
        != "INFRASTRUCTURE_ONLY_AFTER_ZERO_OPTIMIZER_UPDATES"
        or recovery.get("disclosed_v1_failure")
        != "EXECUTION_PERMIT_EXACT_SESSION_TYPE_CHECK_AFTER_SUCCESSFUL_PARENT_HANDSHAKE_DUE_TO_DUPLICATE_MODULE_IDENTITY"
        or recovery.get("same_scientific_coordinate_as_v1") is not True
        or recovery.get("same_scientific_protocol_as_v1") is not True
        or recovery.get("v1_optimizer_updates") != 0
        or recovery.get("v1_checkpoint_exists") is not False
        or recovery.get("v1_retry_resume_or_replacement_authorized") is not False
        or recovery.get("v2_attempt_count") != 1
        or recovery.get("no_warm_start") is not True
        or recovery.get("no_optimizer_state_reuse") is not True
        or recovery.get("no_checkpoint_reuse") is not True
        or recovery.get("no_dataset_or_preflight_cache_reuse") is not True
        or recovery.get("confirmatory_effect") is not False
        or recovery.get("production_order_effect") is not False
        or recovery.get("qualifies_r1") is not False
        or recovery.get("qualifies_r2") is not False
        or recovery.get("closes_c17") is not False
        or recovery.get("claim_promotion_effect") is not False
    ):
        raise DevelopmentCheckpointRefusal("V2 infrastructure amendment changed")
    return freeze
...
def _validate_retained_authorized_freeze(
    ...
    if (
        ...
        or binding.get("isolated_runner_source_path") != ISOLATED_RUNNER_RELATIVE_PATH
        or binding.get("module_identity_test_path")
        != MODULE_IDENTITY_TEST_RELATIVE_PATH
        ...
        or binding.get("isolated_runner_source_sha256")
        != receipt.get("isolated_runner_source_sha256")
        or binding.get("module_identity_test_sha256")
        != receipt.get("module_identity_test_sha256")
    ...
    custody = freeze.get("v1_failure_custody")
    if (
        type(custody) is not dict
        or custody != receipt.get("v1_failure_custody")
        or _v1_failure_custody_sha256(custody)
        != receipt.get("v1_failure_custody_sha256")
    ):
        raise DevelopmentCheckpointRefusal("retained V1 failure custody differs")
...
def _retain_authorization_evidence(
    ...
    identity_test_payload = _read_regular_bytes(
        root / MODULE_IDENTITY_TEST_RELATIVE_PATH, maximum=64 * 1024 * 1024
    )
    if (
        _sha256_bytes(freeze_payload) != bindings["freeze_sha256"]
        or _sha256_bytes(test_payload) != bindings["runner_test_sha256"]
        or _sha256_bytes(identity_test_payload)
        != bindings["module_identity_test_sha256"]
    ):
        raise DevelopmentCheckpointRefusal(
    ...
    _write_retained_bytes(
        destination / RETAINED_MODULE_IDENTITY_TEST_FILE_NAME,
...
def _require_retained_authorization_evidence(
    ...
    identity_test_path = artifact_root / RETAINED_MODULE_IDENTITY_TEST_FILE_NAME
    ...
    identity_test_payload = _read_regular_bytes(
        identity_test_path, maximum=64 * 1024 * 1024
    )
    if (
        _sha256_bytes(freeze_payload) != bindings["freeze_sha256"]
        or _sha256_bytes(test_payload) != bindings["runner_test_sha256"]
        ...
        or identity_test_path.stat().st_mode
...
def _verify_capsule_source_manifest(
    ...
    if (
        ...
        != "heterodiff-a1-development-source-capsule-manifest-v2"
...
def _root_binding_snapshot(root: Path) -> Dict[str, str]:
    v1_custody = _require_v1_failure_custody(root)
    return {
        ...
        "isolated_runner_source_sha256": _sha256_file(
            root / ISOLATED_RUNNER_RELATIVE_PATH, maximum=64 * 1024 * 1024
        ),
        "module_identity_test_sha256": _sha256_file(
            root / MODULE_IDENTITY_TEST_RELATIVE_PATH, maximum=64 * 1024 * 1024
        ),
        "v1_failure_custody_sha256": _v1_failure_custody_sha256(v1_custody),
...
def _wait_for_child_with_limits(
    ...
    *,
    ...
    try:
        while True:
            _require_unchanged_root_bindings(workspace_root, expected_bindings)
            _require_forbidden_outputs_absent(workspace_root)
            remaining = deadline - time.monotonic()
            if remaining <= 0.0:
...
def _artifact_inventory_sha256(rows: Sequence[Mapping[str, object]]) -> str:
    return _sha256_bytes(
        b"heterodiff-a1-development-artifact-inventory-v2\0"
        + _canonical_json_bytes(list(rows))
    )


def _v1_failure_custody_sha256(value: Mapping[str, object]) -> str:
    return _sha256_bytes(
        b"heterodiff-a1-development-v1-failure-custody-binding-v1\0"
        + _canonical_json_bytes(value)
    )


def _require_v1_failure_custody(workspace_root: Path) -> Dict[str, object]:
    """Reopen the complete, immutable V1 failure before V2 may proceed."""

    artifact_root = workspace_root / V1_ARTIFACT_RELATIVE_PATH
    try:
        root_status = artifact_root.lstat()
        resolved_root = artifact_root.resolve(strict=True)
    except OSError as error:
        raise DevelopmentCheckpointRefusal(
            "the immutable V1 failure root is absent"
        ) from error
    if (
        not stat.S_ISDIR(root_status.st_mode)
        or artifact_root.is_symlink()
        or resolved_root != artifact_root.absolute()
    ):
        raise DevelopmentCheckpointRefusal("the immutable V1 failure root is unsafe")

    inventory = _artifact_inventory(artifact_root)
    inventory_sha256 = _sha256_bytes(
        V1_ARTIFACT_INVENTORY_DOMAIN + _canonical_json_bytes(list(inventory))
    )
    total_bytes = sum(int(row["bytes"]) for row in inventory)
    checkpoint_files = tuple(
        str(row["path"])
        for row in inventory
        if Path(str(row["path"])).suffix.lower() == ".pt"
    )
    if (
        len(inventory) != V1_EXPECTED_REGULAR_FILE_COUNT
        or total_bytes != V1_EXPECTED_TOTAL_REGULAR_FILE_BYTES
        or inventory_sha256 != V1_EXPECTED_ARTIFACT_INVENTORY_SHA256
        or checkpoint_files
    ):
        raise DevelopmentCheckpointRefusal("immutable V1 artifact inventory changed")

    failure_path = artifact_root / V1_FAILURE_RECEIPT_RELATIVE_PATH
    failure_payload = _read_regular_bytes(failure_path, maximum=_MAXIMUM_JSON_BYTES)
    if _sha256_bytes(failure_payload) != V1_EXPECTED_FAILURE_RECEIPT_RAW_SHA256:
        raise DevelopmentCheckpointRefusal("immutable V1 failure receipt changed")
    failure = _parse_json_object(failure_payload)
    checked_failure = _validate_self_digested_record(
        failure, field="record_sha256", name="V1 failure receipt"
    )
    if (
        checked_failure.get("record_sha256")
        != V1_EXPECTED_FAILURE_RECEIPT_RECORD_SHA256
        or checked_failure.get("schema")
        != "heterodiff-a1-development-attempt-terminal-v1"
        or checked_failure.get("lane_id") != "A1-DEV-GUIDED-1729-N32768-V1"
        or checked_failure.get("state") != "FAILURE"
        or checked_failure.get("child_returncode") != 1
        or checked_failure.get("checkpoint_claimed") is not False
        or checked_failure.get("retry_permitted") is not False
        or checked_failure.get("replacement_permitted") is not False
        or checked_failure.get("scientific_result_eligible") is not False
        or checked_failure.get("production_order_admissible") is not False
        or checked_failure.get("qualifies_r1") is not False
        or checked_failure.get("qualifies_r2") is not False
        or checked_failure.get("claim_promotion") is not False
    ):
        raise DevelopmentCheckpointRefusal("immutable V1 failure semantics changed")

    ledger_path = artifact_root / V1_LEDGER_RELATIVE_PATH
    ledger_payload = _read_regular_bytes(ledger_path, maximum=_MAXIMUM_JSON_BYTES)
    if _sha256_bytes(ledger_payload) != V1_EXPECTED_LEDGER_RAW_SHA256:
        raise DevelopmentCheckpointRefusal("immutable V1 ledger changed")
    ledger = _parse_json_object(ledger_payload)
    ledger_without_digest = dict(ledger)
    ledger_digest = _require_sha256(
        ledger_without_digest.pop("ledger_sha256", None), name="V1 ledger_sha256"
    )
    runs = ledger.get("runs")
    if (
        ledger_digest != V1_EXPECTED_LEDGER_RECORD_SHA256
        or _sha256_bytes(_canonical_json_bytes(ledger_without_digest)) != ledger_digest
        or ledger.get("schema") != "heterodiff-a1-exactly-once-ledger-v5"
        or type(runs) is not dict
        or len(runs) != 1
    ):
        raise DevelopmentCheckpointRefusal("immutable V1 ledger semantics changed")
    run = next(iter(runs.values()))
    if (
        type(run) is not dict
        or run.get("state") != "FAILURE"
        or run.get("failed_stage") != "PREPARED"
        or run.get("previous_state") != "PREPARED"
        or run.get("error_type") != "TypeError"
        or run.get("error_message")
        != "execution permit requires a parent-handshaken session"
        or any(
            name in run
            for name in (
                "optimizer_steps_taken",
        ...
        raise DevelopmentCheckpointRefusal("V1 is not the frozen zero-update failure")

    stderr_path = artifact_root / V1_INNER_STDERR_RELATIVE_PATH
    if (
        _sha256_file(stderr_path, maximum=_MAXIMUM_JSON_BYTES)
        != V1_EXPECTED_INNER_STDERR_RAW_SHA256
    ):
        raise DevelopmentCheckpointRefusal("immutable V1 stderr changed")
    machine_freeze_path = workspace_root / V1_MACHINE_FREEZE_RELATIVE_PATH
    if (
        _sha256_file(machine_freeze_path, maximum=_MAXIMUM_JSON_BYTES)
        != V1_EXPECTED_MACHINE_FREEZE_RAW_SHA256
        or _sha256_file(
            artifact_root / V1_RETAINED_FREEZE_RELATIVE_PATH,
            maximum=_MAXIMUM_JSON_BYTES,
        )
        != V1_EXPECTED_MACHINE_FREEZE_RAW_SHA256
    ):
        raise DevelopmentCheckpointRefusal("immutable V1 freeze custody changed")
    for forbidden in (
        "success-receipt.json",
        "execution-outcome-linkage.json",
        "capsule/artifacts/a1_campaign_v4/success-receipt.json",
    ):
        if _lexists(artifact_root / forbidden):
            raise DevelopmentCheckpointRefusal(
                "V1 unexpectedly contains success evidence"
            )

    return {
        "schema": "heterodiff-a1-development-v1-failure-custody-binding-v1",
        "lane_id": "A1-DEV-GUIDED-1729-N32768-V1",
        "artifact_root": V1_ARTIFACT_RELATIVE_PATH,
        "artifact_inventory_domain": "heterodiff-a1-development-artifact-inventory-v1",
        "regular_file_count": len(inventory),
        "total_regular_file_bytes": total_bytes,
        "artifact_inventory_sha256": inventory_sha256,
        "failure_receipt_path": V1_ARTIFACT_RELATIVE_PATH
        + "/"
        + V1_FAILURE_RECEIPT_RELATIVE_PATH,
        "failure_receipt_raw_sha256": V1_EXPECTED_FAILURE_RECEIPT_RAW_SHA256,
        "failure_receipt_record_sha256": V1_EXPECTED_FAILURE_RECEIPT_RECORD_SHA256,
        "ledger_path": V1_ARTIFACT_RELATIVE_PATH + "/" + V1_LEDGER_RELATIVE_PATH,
        "ledger_raw_sha256": V1_EXPECTED_LEDGER_RAW_SHA256,
        "ledger_record_sha256": V1_EXPECTED_LEDGER_RECORD_SHA256,
        "inner_stderr_path": V1_ARTIFACT_RELATIVE_PATH
        + "/"
        + V1_INNER_STDERR_RELATIVE_PATH,
        "inner_stderr_raw_sha256": V1_EXPECTED_INNER_STDERR_RAW_SHA256,
        "machine_freeze_path": V1_MACHINE_FREEZE_RELATIVE_PATH,
        "machine_freeze_raw_sha256": V1_EXPECTED_MACHINE_FREEZE_RAW_SHA256,
        "retained_authorized_freeze_path": V1_ARTIFACT_RELATIVE_PATH
        + "/"
        + V1_RETAINED_FREEZE_RELATIVE_PATH,
        "retained_authorized_freeze_raw_sha256": V1_EXPECTED_MACHINE_FREEZE_RAW_SHA256,
        "state": "FAILURE",
        "failed_stage": "PREPARED",
        "child_returncode": 1,
...
def _validate_permit_consumption(
    ...
    *,
    ...
    if (
        ...
        or record.get("schema") != "heterodiff-a1-development-permit-consumption-v2"
        ...
        or record.get("launch_gate_protocol") != "PIPE_EOF_FAIL_CLOSED_V2"
...
def _validate_outcome_linkage(
    ...
    *,
    ...
    if (
        ...
        or record.get("schema") != "heterodiff-a1-development-outcome-linkage-v2"
        or record.get("lane_id") != LANE_ID
...
def _terminal_custody(artifact_root: Path) -> Dict[str, object]:
    ...
    paths = {
        ...
        "retained_module_identity_test_sha256": artifact_root
        / RETAINED_MODULE_IDENTITY_TEST_FILE_NAME,
    ...
    result["v1_failure_custody_sha256"] = _v1_failure_custody_sha256(
        _require_v1_failure_custody(workspace_root)
    )
    return result
...
def _terminal_record(
    *,
    ...
    record: Dict[str, object] = {
        "schema": "heterodiff-a1-development-attempt-terminal-v2",
        "lane_id": LANE_ID,
        "state": state,
        ...
        "partial_custody": partial_custody,
        "boundary_drift": dict(boundary_drift or {}),
        "v1_failure_custody_sha256": partial_custody.get("v1_failure_custody_sha256"),
        "retry_permitted": False,
        "replacement_permitted": False,
...
def _validate_terminal_receipt(value: Mapping[str, object]) -> Dict[str, object]:
    ...
    if (
        record.get("schema") != "heterodiff-a1-development-attempt-terminal-v2"
        or record.get("lane_id") != LANE_ID
        ...
        or record.get("confirmatory_execution") is not False
        or record.get("qualifies_r1") is not False
        or record.get("qualifies_r2") is not False
        or record.get("closes_c17") is not False
        or record.get("claim_promotion") is not False
        or record.get("real_domain_test_accessed") is not False
    ):
    ...
    if record.get("v1_failure_custody_sha256") != custody.get(
        "v1_failure_custody_sha256"
    ):
        raise DevelopmentCheckpointRefusal("terminal V1 failure custody changed")
...
def _validate_success_receipt(value: Mapping[str, object]) -> Dict[str, object]:
    ...
    v1_custody = receipt.get("v1_failure_custody")
    if (
        type(v1_custody) is not dict
        or _v1_failure_custody_sha256(v1_custody)
        != receipt.get("v1_failure_custody_sha256")
        or v1_custody.get("artifact_inventory_sha256")
        != V1_EXPECTED_ARTIFACT_INVENTORY_SHA256
        or v1_custody.get("failure_receipt_raw_sha256")
        != V1_EXPECTED_FAILURE_RECEIPT_RAW_SHA256
        or v1_custody.get("failure_receipt_record_sha256")
        != V1_EXPECTED_FAILURE_RECEIPT_RECORD_SHA256
        or v1_custody.get("ledger_raw_sha256") != V1_EXPECTED_LEDGER_RAW_SHA256
        or v1_custody.get("inner_stderr_raw_sha256")
        != V1_EXPECTED_INNER_STDERR_RAW_SHA256
        or v1_custody.get("machine_freeze_raw_sha256")
        != V1_EXPECTED_MACHINE_FREEZE_RAW_SHA256
        or v1_custody.get("optimizer_updates") != 0
        or v1_custody.get("checkpoint_file_count") != 0
        or v1_custody.get("checkpoint_present") is not False
    ):
        raise DevelopmentCheckpointRefusal("success V1 failure custody changed")
    inner = receipt.get("inner_success")
    if (
        type(inner) is not dict
        or set(inner) != _INNER_SUCCESS_FIELDS
        or inner.get("schema") != "heterodiff-a1-development-inner-success-summary-v2"
    ...
    if (
        receipt.get("retained_authorized_freeze_sha256") != receipt.get("freeze_sha256")
        or receipt.get("retained_runner_test_sha256")
        != receipt.get("runner_test_sha256")
        or receipt.get("retained_module_identity_test_sha256")
        != receipt.get("module_identity_test_sha256")
    ):
        raise DevelopmentCheckpointRefusal(
    ...
    if (
        inventory_by_path.get(RETAINED_FREEZE_FILE_NAME, {}).get("sha256")
        != receipt.get("freeze_sha256")
        or inventory_by_path.get(RETAINED_RUNNER_TEST_FILE_NAME, {}).get("sha256")
        != receipt.get("runner_test_sha256")
        or inventory_by_path.get(RETAINED_MODULE_IDENTITY_TEST_FILE_NAME, {}).get(
            "sha256"
        )
        != receipt.get("module_identity_test_sha256")
    ):
...
def _revalidate_durable_success(
    ...
    *,
    ...
    current_v1_custody = _require_v1_failure_custody(workspace_root)
    if (
        current_v1_custody != checked["v1_failure_custody"]
        or _v1_failure_custody_sha256(current_v1_custody)
        != checked["v1_failure_custody_sha256"]
    ):
        raise DevelopmentCheckpointRefusal("durable V1 failure custody differs")
    ...
    retained_identity_test_payload = _read_regular_bytes(
        artifact_root / RETAINED_MODULE_IDENTITY_TEST_FILE_NAME,
        maximum=64 * 1024 * 1024,
    )
    if (
        ...
        or _sha256_bytes(retained_identity_test_payload)
        != checked["module_identity_test_sha256"]
    ):
        raise DevelopmentCheckpointRefusal(
    ...
    isolated_runner_rows = [
        row
        for row in manifest["files"]
        if type(row) is dict and row.get("path") == ISOLATED_RUNNER_RELATIVE_PATH
    ]
    if (
        len(isolated_runner_rows) != 1
        or isolated_runner_rows[0].get("sha256")
        != checked["isolated_runner_source_sha256"]
    ):
        raise DevelopmentCheckpointRefusal(
            "retained capsule isolated runner differs from receipt"
        )
    ...
    if (
        ...
        or permit.get("schema") != "heterodiff-a1-development-single-use-permit-v2"
        ...
        or permit.get("v1_failure_custody_sha256")
        != checked["v1_failure_custody_sha256"]
        or permit.get("isolated_runner_source_sha256")
        != checked["isolated_runner_source_sha256"]
        or permit.get("module_identity_test_sha256")
        != checked["module_identity_test_sha256"]
    ...
    if (
        ...
        or attempt.get("schema") != "heterodiff-a1-development-attempt-v2"
        ...
        or attempt.get("v1_failure_custody_sha256")
        != checked["v1_failure_custody_sha256"]
    ...
    fresh = _run_json_subprocess(
        (
            ...
            "-P",
            "-m",
            "heterodiff.experiments.finite_association_development_checkpoint_runner_v2",
    ...
    _require_saved_json_equals(artifact_root / "success-receipt.json", checked)
    final_v1_custody = _require_v1_failure_custody(workspace_root)
    if final_v1_custody != checked["v1_failure_custody"]:
        raise DevelopmentCheckpointRefusal("V1 failure custody changed during status")
...
def execute_development_checkpoint() -> Dict[str, object]:
    ...
    if (
        ...
        or implementation_binding.get("isolated_runner_source_sha256")
        != root_bindings["isolated_runner_source_sha256"]
        or implementation_binding.get("module_identity_test_sha256")
        != root_bindings["module_identity_test_sha256"]
    ):
        raise DevelopmentCheckpointRefusal(
            "authorized freeze or bound implementation changed during preflight"
        )
    python = _target_python(root)
    temporary = Path(tempfile.mkdtemp(prefix=".a1-development-v2-", dir=str(artifacts)))
    published = False
    process = None
    ...
    try:
        ...
        permit: Dict[str, object] = {
            "schema": "heterodiff-a1-development-single-use-permit-v2",
            "lane_id": LANE_ID,
            "state": "ISSUED",
            "coordinate": {"seed": SEED, "budget": BUDGET, "method": METHOD},
            "v1_failure_custody_sha256": root_bindings["v1_failure_custody_sha256"],
            ...
            "isolated_runner_source_sha256": root_bindings[
                "isolated_runner_source_sha256"
            ],
            "module_identity_test_sha256": root_bindings["module_identity_test_sha256"],
        ...
        prepared = {
            "schema": "heterodiff-a1-development-attempt-v2",
            "lane_id": LANE_ID,
            "state": "PREPARED",
            "started_unix_ns": started_unix_ns,
            "coordinate": {"seed": SEED, "budget": BUDGET, "method": METHOD},
        ...
        with open(stdout_path, "xb") as stdout_handle, open(
            ...
            try:
                ...
                consumption = {
                    "schema": "heterodiff-a1-development-permit-consumption-v2",
                    "lane_id": LANE_ID,
                    ...
                    "launch_gate_protocol": "PIPE_EOF_FAIL_CLOSED_V2",
            ...
            returncode, timed_out = _wait_for_child_with_limits(
                ...
                workspace_root=root,
                expected_bindings=root_bindings,
                deadline=deadline,
            )
            child_wait_completed = True
        ...
        outcome_linkage: Dict[str, object] = {
            "schema": "heterodiff-a1-development-outcome-linkage-v2",
            "lane_id": LANE_ID,
        ...
        final_v1_custody = _require_v1_failure_custody(root)
        receipt: Dict[str, object] = {
            ...
            "finished_unix_ns": time.time_ns(),
            "coordinate": {"seed": SEED, "budget": BUDGET, "method": METHOD},
            "v1_failure_custody": final_v1_custody,
            "v1_failure_custody_sha256": _v1_failure_custody_sha256(final_v1_custody),
            "freeze_sha256": root_bindings["freeze_sha256"],
            ...
            "retained_module_identity_test_sha256": root_bindings[
                "module_identity_test_sha256"
            ],
            "runner_source_sha256": root_bindings["runner_source_sha256"],
            "runner_test_sha256": root_bindings["runner_test_sha256"],
...
def development_checkpoint_status() -> Dict[str, object]:
    ...
    _require_v1_failure_custody(workspace)
