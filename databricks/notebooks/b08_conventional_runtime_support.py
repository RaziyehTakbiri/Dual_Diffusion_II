"""Definitions for the managed Databricks B08 notebook; importing runs no work."""
...
SUPPORT_RELATIVE_PATH = "databricks/notebooks/b08_conventional_runtime_support.py"
MANAGED_RUNTIME_ROOT = Path("/tmp/heterodiff-b08-managed-current")
MANAGED_CONTEXT_SCHEMA = "heterodiff-b08-databricks-percent-pip-v1"
MANAGED_WHEEL_NAME = "heterodiff-0.1.0-py3-none-any.whl"
...
def _load_controller_anchor(project_root: Path) -> dict[str, Any]:
    ...
    support_binding = anchor["support_module"]
    if (
        not isinstance(support_binding, dict)
        or set(support_binding) != {"relative_path", "sha256", "size_bytes"}
        or support_binding["relative_path"] != SUPPORT_RELATIVE_PATH
    ):
        raise B08ConventionalRuntimeError("SUPPORT_MODULE_BINDING_INVALID")
    support_path = project_root / SUPPORT_RELATIVE_PATH
    if support_path.is_symlink() or _file_content_binding(support_path) != {
        "sha256": support_binding["sha256"], "size_bytes": support_binding["size_bytes"]
    }:
        raise B08ConventionalRuntimeError("SUPPORT_MODULE_BINDING_MISMATCH")
    ...
    return {
        ...
        "support_module": dict(support_binding),
    }
...
def _managed_identity(preflight: Mapping[str, Any]) -> dict[str, Any]:
    manifest = preflight["source_manifest"]
    return {
        "schema_version": MANAGED_CONTEXT_SCHEMA,
        "controller_anchor": preflight["controller_anchor"],
        "source_manifest_relative_path": manifest["relative_path"],
        "source_manifest_file_sha256": manifest["file_sha256"],
        "source_manifest_record_sha256": manifest["record_sha256"],
        "lock_sha256": preflight["lock_sha256"],
    }


def _write_managed_context(context: Mapping[str, Any]) -> None:
    (MANAGED_RUNTIME_ROOT / "context.json").write_bytes(_canonical_json_bytes(context) + b"\n")


def _load_managed_context(preflight: Mapping[str, Any], phase: str, *, restarted: bool = False) -> dict[str, Any]:
    context = _read_json(MANAGED_RUNTIME_ROOT / "context.json")
    expected = {**_managed_identity(preflight), "phase": phase}
    mismatches = {
        key: {"expected": value, "observed": context.get(key)}
        for key, value in expected.items() if context.get(key) != value
    }
    if mismatches:
        raise B08ConventionalRuntimeError("MANAGED_CONTEXT_BINDING_MISMATCH", diagnostics=mismatches)
    if _sha256_file(MANAGED_RUNTIME_ROOT / "dependencies.lock") != preflight["lock_sha256"]:
        raise B08ConventionalRuntimeError("MANAGED_STAGED_LOCK_MISMATCH")
    _verify_source_snapshot(Path(context["staging_root"]) / "source", preflight["source_manifest"])
    if restarted:
        previous_pid = context.get("pre_restart_pid")
        if type(previous_pid) is not int or previous_pid == os.getpid():
            raise B08ConventionalRuntimeError("MANAGED_PYTHON_RESTART_NOT_OBSERVED")
    return context


def _prepare_managed_run(project_root: Path, preflight: Mapping[str, Any]) -> dict[str, Any]:
    # This route has its own context. Never read, reuse, or delete the old
    # conventional subprocess-install marker or Candidate 002/003 files.
    if MANAGED_RUNTIME_ROOT.is_symlink():
        raise B08ConventionalRuntimeError("MANAGED_RUNTIME_ROOT_IS_SYMLINK")
    MANAGED_RUNTIME_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    staging_root = Path(tempfile.mkdtemp(prefix="heterodiff-b08-managed-", dir="/tmp"))
    _stage_source_snapshot(project_root, preflight["source_manifest"], staging_root)
    lock_payload = (project_root / LOCK_RELATIVE_PATH).read_bytes()
    if _sha256_bytes(lock_payload) != preflight["lock_sha256"]:
        raise B08ConventionalRuntimeError("LOCK_CHANGED_BEFORE_MANAGED_INSTALL")
    (MANAGED_RUNTIME_ROOT / "dependencies.lock").write_bytes(lock_payload)
    context = {
        **_managed_identity(preflight),
        "phase": "PREPARED",
        "staging_root": str(staging_root),
        "python_prefix_observations": [],
    }
    _write_managed_context(context)
    return {"decision": "READY_FOR_DATABRICKS_PERCENT_PIP", "phase": context["phase"]}


def _record_managed_restart(preflight: Mapping[str, Any], *, dependencies: bool) -> dict[str, Any]:
    before = "PREPARED" if dependencies else "PROJECT_WHEEL_READY"
    after = "DEPENDENCIES_INSTALLED" if dependencies else "PROJECT_INSTALLED"
    context = _load_managed_context(preflight, before)
    if not dependencies:
        _verify_managed_wheel(context)
    context["phase"] = after
    context["pre_restart_pid"] = os.getpid()
    context["python_prefix_observations"].append({
        "phase": after, "prefix": sys.prefix, "pid": os.getpid(),
    })
    _write_managed_context(context)
    return {"decision": "DATABRICKS_PYTHON_RESTART_REQUESTED", "phase": after}


def _verify_managed_dependencies(preflight: Mapping[str, Any]) -> None:
    mismatches = {}
    for name, expected in preflight["lock_versions"].items():
        try:
    ...
    if mismatches:
        raise B08ConventionalRuntimeError(
            "MANAGED_DEPENDENCIES_NOT_AVAILABLE_AFTER_RESTART",
            diagnostics={"packages": mismatches, "python_prefix": sys.prefix},
        )


def _verify_managed_wheel(context: Mapping[str, Any]) -> None:
    if context.get("project_wheel_name") != MANAGED_WHEEL_NAME:
        raise B08ConventionalRuntimeError("MANAGED_PROJECT_WHEEL_NAME_INVALID")
    staged_wheel = Path(context["staging_root"]) / "wheel" / MANAGED_WHEEL_NAME
    install_wheel = MANAGED_RUNTIME_ROOT / MANAGED_WHEEL_NAME
    if any(_sha256_file(path) != context["project_wheel_sha256"] for path in (staged_wheel, install_wheel)):
        raise B08ConventionalRuntimeError("MANAGED_PROJECT_WHEEL_BINDING_MISMATCH")


def _build_managed_project(project_root: Path, preflight: Mapping[str, Any]) -> dict[str, Any]:
    context = _load_managed_context(preflight, "DEPENDENCIES_INSTALLED", restarted=True)
    _verify_managed_dependencies(preflight)
    ...
    environment.update({
        ...
        "SOURCE_DATE_EPOCH": str(max(315532800, int(preflight["source_manifest"]["record_sha256"][:8], 16))),
    })
    # A subprocess is suitable for building, never for installing into the
    # managed notebook environment. Both installs use literal %pip cells.
    _run((sys.executable, "-m", "pip", "wheel", "--no-index", "--no-deps",
          "--no-build-isolation", "--wheel-dir", str(wheel_root),
          str(staging_root / "source")), cwd=staging_root,
         timeout_seconds=900, environment=environment)
    wheels = list(wheel_root.glob("*.whl"))
    if len(wheels) != 1 or wheels[0].name != MANAGED_WHEEL_NAME:
        raise B08ConventionalRuntimeError("MANAGED_PROJECT_WHEEL_COUNT_OR_NAME_INVALID")
    wheel_path = wheels[0]
    shutil.copyfile(wheel_path, MANAGED_RUNTIME_ROOT / MANAGED_WHEEL_NAME)
    context.update({
        "phase": "PROJECT_WHEEL_READY", "project_wheel_name": wheel_path.name,
        "project_wheel_sha256": _sha256_file(wheel_path),
    })
    _verify_managed_wheel(context)
    _write_managed_context(context)
    return {"decision": "READY_FOR_MANAGED_PROJECT_INSTALL", "project_wheel_sha256": context["project_wheel_sha256"]}
...
def _verify_and_integrate(
    ...
    receipt = {
        ...
        "managed_installation": {
            "method": "DATABRICKS_PERCENT_PIP",
            "restart_count": len(marker["python_prefix_observations"]),
            "python_prefix_changed": any(
                observation["prefix"] != sys.prefix
                for observation in marker["python_prefix_observations"]
            ),
            "environment_verified_by": "LOCKED_VERSIONS_IMPORT_ORIGINS_AND_PIP_CHECK",
        },
...
def managed_action(action: str) -> dict[str, Any]:
    project_root = None
    try:
        project_root = _find_project_root()
        preflight = _runtime_preflight(project_root)
        if action == "prepare":
            result = _prepare_managed_run(project_root, preflight)
        elif action == "dependencies_restart":
            result = _record_managed_restart(preflight, dependencies=True)
        elif action == "build":
            result = _build_managed_project(project_root, preflight)
        elif action == "project_restart":
            result = _record_managed_restart(preflight, dependencies=False)
        elif action == "verify":
            context = _load_managed_context(preflight, "PROJECT_INSTALLED", restarted=True)
            _verify_managed_wheel(context)
            _verify_managed_dependencies(preflight)
            result = _verify_and_integrate(project_root, preflight, context)
        else:
            raise B08ConventionalRuntimeError("UNKNOWN_MANAGED_ACTION")
        ...
        failure = {
            ...
            "safety": {"study_or_test_data_accessed": False, "training_executed": False,
                       "calibration_executed": False, "inference_executed": False},
        }
